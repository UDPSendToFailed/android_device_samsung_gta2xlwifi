...
class FlashService : Service() {
  ...
  private fun getBrightness(mCtx: Context) =
    PreferenceManager.getDefaultSharedPreferences(mCtx)
      .getInt(FlashFragment.PREF_FLASH_INTENSITY, 1)
  ...
  private val mFlashObserver =
    object : ContentObserver(mMainHandler) {
      override fun onChange(s: Boolean) {
        super.onChange(s)
        val isOn =
          Settings.Secure.getInt(mContext.contentResolver, Settings.Secure.FLASHLIGHT_ENABLED, 0)
        if (isOn == 1) {
